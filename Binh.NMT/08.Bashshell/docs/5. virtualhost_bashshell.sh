...
sudo mkdir /var/www/thanhbinh
# Gan quyen cho thu muc
sudo chown -R $USER:$USER /var/www/thanhbinh
sudo chown -R 755 /var/www/thanhbinh
# Tao mot trang mau index
sudo tee /var/www/thanhbinh/index.html <<EOF
<html>
    <head>
        <title>Welcome to thanhbinh!</title>
    </head>
    <body>
        <h1>Success!  The thanhbinh virtual host is working!</h1>
    </body>
</html>
EOF
# Tao 1 file virtual host
sudo tee /etc/apache2/sites-available/thanhbinh.conf <<EOF
<VirtualHost *:80>
